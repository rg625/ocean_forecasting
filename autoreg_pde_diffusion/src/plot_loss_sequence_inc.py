...
if "obsMask" in groundTruthDict:
    obsMask = (
        groundTruthDict["obsMask"].unsqueeze(1).unsqueeze(2).unsqueeze(0).unsqueeze(0)
    )
    ...
    if metric == "MSE":
        ...
        for i in range(gtReshape.shape[0]):
            stacked = torch.concat(
                [gtReshape[i : i + 1], predReshape[i : i + 1]], dim=0
            )
...
for i in range(len(modelNames)):
    ...
    ax.plot(
        np.arange(distanceMean[i].shape[0]) + 2,
        distanceMean[i],
        linewidth=1.5,
        color=color,
        label=label,
