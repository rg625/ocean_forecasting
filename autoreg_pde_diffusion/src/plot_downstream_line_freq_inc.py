...
for modelName, modelPath in models.items():
    ...
    lineField = prediction[
        :,
        :,
        :,
        :,
        getFieldIndex(datasetName, field) : getFieldIndex(datasetName, field) + 1,
        linePos : linePos + 1,
    ]
    ...
    if datasetName in ["extrap", "interp", "longer"]:
        gridSpacing = (
            6.0 / fft.shape[6]
        )  # spacing determined by 12x6 interpolation area
    elif datasetName in ["zInterp"]:
        gridSpacing = 2 * (
            (2 * 3.1415) / 1024
        )  # spacing determined by 2pi x 2pi full simulation area of resolution 1024x1024 with strided queries of 2
    else:
        gridSpacing = 1
...
ax.text(
    ...
    color="k",
    bbox=dict(facecolor="whitesmoke", edgecolor="darkslategray", boxstyle="round"),
...
if datasetName in ["extrap", "interp", "longer", "highRey", "lowRey", "varReyIn"]:
    ...
    ax.set_xlabel(
        "Wavenumber of %s for Flow along Vertical Line at x Position %1.2f"
        % (field, cylsDownstream * prediction.shape[5] * 2 * ((2 * 3.1415) / 1024))
    )
...
if withInset:
    ...
    axIns = ax.inset_axes(
        [0.29, 0.08, 0.45, 0.50], xlim=(0.42, 0.492), ylim=(10 ** (-7.9), 10 ** (-5.3))
    )
...
for i in range(len(modelNames)):
    ...
    if modelNames[i] == "Simulation":
        ...
        if withInset:
            axIns.plot(
                freq, lineMean[i], linewidth=1.5, color=color, linestyle="dotted"
            )
