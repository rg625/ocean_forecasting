...
if __name__ == "__main__":
    ...
    trainSet = TurbulenceDataset(
        ...
        ["data"],
        filterTop=["128_tra"],
        filterSim=[[0, 1, 2, 14, 15, 16, 17, 18]],
...
def train(
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=p_t.lr, weight_decay=p_t.weightDecay
    )
    ...
    trainer = TrainerDiffusion(
        model, trainLoader, optimizer, trainHistory, logger.tfWriter, p_t
    )
    ...
    for epoch in range(0, p_t.epochs):
        ...
        trainHistory.updateAccuracy(
            [p_d, p_t, p_l, p_me, p_md, p_ml], testHistories, epoch == p_t.epochs - 1
        )
