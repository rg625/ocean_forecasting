...
if not load:
    ...
    for datasetName in datasets:
        ...
        print(
            "%s:   Original ground truth shape: %s"
            % (datasetName, str(list(groundTruth.shape)))
        )
        groundTruth = groundTruth[
            ...
            getLossRelevantFields(datasetName)[0] : getLossRelevantFields(datasetName)[
                1
            ],
        ]
        print(
            "%s:   Loaded ground truth with shape: %s"
            % (datasetName, str(list(groundTruth.shape)))
        )
        ...
        for modelName, modelPath in models.items():
            ...
            prediction = prediction[
                ...
                sequenceMinMax[0] : sequenceMinMax[1],
                timeMinMax[0] : timeMinMax[1],
                getLossRelevantFields(datasetName)[0] : getLossRelevantFields(
                    datasetName
                )[1],
            ]
            print(
                "%s:   Loaded prediction from model %s with shape: %s"
                % (datasetName, modelName, str(list(prediction.shape)))
            )

            if metric == "MSE":
                mse = F.mse_loss(
                    groundTruth.expand_as(prediction), prediction, reduction="none"
                )
                mseScalar = torch.mean(mse, dim=(3, 4, 5, 6))
                distanceMean[datasetName] += [
                    rescaleMap[datasetName] * torch.mean(mseScalar).numpy()
                ]
                distanceStd[datasetName] += [
                    rescaleMap[datasetName] * torch.std(mseScalar).numpy()
                ]
                ...
                gt = torch.reshape(
                    gt, [-1, gt.shape[3], gt.shape[4], gt.shape[5], gt.shape[6]]
                )
                lsim = []
                ...
                distanceMean[datasetName] += [
                    rescaleMap[datasetName] * torch.mean(lsimScalar).numpy()
                ]
                distanceStd[datasetName] += [
                    rescaleMap[datasetName] * torch.std(lsimScalar).numpy()
                ]
    ...
    for j in range(len(datasets)):
        print(
            ...
            % (
                "{:<4}".format(metric),
                "{:<7}".format(datasets[j]),
                "{:<10}".format(modelNames[datasets[j]][i]),
                distanceMean[datasets[j]][i],
                distanceStd[datasets[j]][i],
            )
        )
