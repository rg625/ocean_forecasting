...
for modelName, modelPath in models.items():
    ...
    n = min(
        fftX.shape[5], fftY.shape[6]
    )  # minimal number of frequencies in fourier space
    ...
    energyX = torch.mean(
        energyX[:, :, :, :, :, 1 : int(n / 2)], dim=6
    )  # only use positive fourier frequencies
    energyY = torch.mean(energyY[:, :, :, :, :, :, 1 : int(n / 2)], dim=5)

    energy = torch.squeeze(0.5 * (energyX + energyY), dim=4)
    energy = torch.mean(
        energy[:, :, :, tkeAverageMinMax[0] : tkeAverageMinMax[1]], dim=3
    )
