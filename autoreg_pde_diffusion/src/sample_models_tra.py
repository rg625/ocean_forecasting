...
testSets = {
    "extrap": TurbulenceDataset(
        "Test Extrapolate Mach 0.50-0.52",
        ["data"],
        filterTop=["128_tra"],
        filterSim=[(0, 3)],
        filterFrame=[(500, 750)],
    ...
    "interp": TurbulenceDataset(
        "Test Interpolate Mach 0.66-0.68",
        ["data"],
        filterTop=["128_tra"],
        filterSim=[(16, 19)],
        filterFrame=[(500, 750)],
    ...
    "longer": TurbulenceDataset(
        "Test Longer Rollout Mach 0.64-0.65",
        ["data"],
        filterTop=["128_tra"],
        filterSim=[(14, 16)],
        filterFrame=[(0, 1000)],
...
dataTransformationFieldIndices = [
    0,
    1,
    2,
    3,
...
for shortTestSetName, testSet in testSets.items():
    ...
    shutil.copy(
        sys.argv[0], os.path.join(testSetOutPath, "sample_models.py")
    )  # copy this script to output folder for reference
    ...
    testLoader = DataLoader(
        testSet, sampler=testSampler, batch_size=len(testSet), drop_last=False
    )
    ...
    for shortTestSetName, testSet in testSets.items():
        ...
        for modelPath in modelPaths:
            model = PredictionModel.load(
                os.path.join(modelFolder, modelPath), useGPU=device == "cuda"
            )
            model.eval()

            if isinstance(model.modelDecoder, DiffusionModel):
                if "samplingMode" in evalOptions:
                    model.modelDecoder.inferenceSamplingMode = evalOptions[
                        "samplingMode"
                    ]
                if "posteriorSampling" in evalOptions:
                    model.modelDecoder.inferencePosteriorSampling = evalOptions[
                        "posteriorSampling"
                    ]
                if "initialSampling" in evalOptions:
                    model.modelDecoder.inferenceInitialSampling = evalOptions[
                        "initialSampling"
                    ]
                if "conditioningIntegration" in evalOptions:
                    model.modelDecoder.inferenceConditioningIntegration = evalOptions[
                ...
                for module in model.modelDecoder:
                    if isinstance(module, DiffusionModel):
                        ...
                        if "posteriorSampling" in evalOptions:
                            module.inferencePosteriorSampling = evalOptions[
                                "posteriorSampling"
                            ]
                        if "initialSampling" in evalOptions:
                            module.inferenceInitialSampling = evalOptions[
                                "initialSampling"
                            ]
                        if "conditioningIntegration" in evalOptions:
                            module.inferenceConditioningIntegration = evalOptions[
                                "conditioningIntegration"
                            ]
            ...
            testSampler = SequentialSampler(testSet)
            testLoader = DataLoader(
                testSet, sampler=testSampler, batch_size=batchSize, drop_last=False
            )  # , num_workers=4)
            ...
            with torch.no_grad():
                ...
                if (
                    numEvalsOption == 1
                    or evalOptions["sequentialEvalRuns"][shortTestSetName]
                ):
                    print(
                        "\tSampling sequentially for %d samples..." % (numEvalsOption)
                    )
                    ...
                    for run in range(numEvalsOption):
                        ...
                        if (
                            numEvalsOption == 1
                        ):  # repeat same evalation result for non-probabilistic models
                            predSamples = np.repeat(predSamples, numEvals, axis=1)

                        # undo data normalization
                        normMean = testTransformations.normMean[
                            dataTransformationFieldIndices
                        ]
                        normStd = testTransformations.normStd[
                            dataTransformationFieldIndices
                        ]
                        ...
                        print(
                            "\t[%2.2f min] Sample %d done."
                            % ((timerEvalsEnd - timerEvalsStart) / 60, run)
                        )
                ...
                # SAMPLE IN PARALLEL
                    ...
                    for s, sample in enumerate(testLoader, 0):
                        dataPath = sample["path"]
                        data = (
                            sample["data"].to(device).repeat(numEvalsOption, 1, 1, 1, 1)
                        )
        ...
        print(
            ...
            % (
                (timerFullEnd - timerFullStart) / 60,
                os.path.join(testSetOutPath, modelName + ".npz"),
                predFull.shape,
            )
        )
        # output shape:
