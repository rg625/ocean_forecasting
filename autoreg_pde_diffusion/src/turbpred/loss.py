...
class PredictionLoss(nn.modules.loss._Loss):
    ...
    def __init__(
        self, p_l: LossParams, dimension: int, simFields: List[str], useGPU: bool
    ):
    ...
    def forward(
        ...
        assert not (
            (self.p_l.predMSE > 0) and prediction.shape[1] <= 1
        ), "Sequence length to small for prediction errors!"
        device = "cuda" if self.useGPU else "cpu"

        if not weighted:
            if self.dimension == 2:
                seqMSE = F.mse_loss(prediction, groundTruth, reduction="none").mean(
                    (0, 2, 3, 4)
                )
            ...
            if self.dimension == 2:
                ...
                if self.p_l.predLSIM > 0:
                    if self.p_l.recLSIM > 0:
                        ...
                        seqLSIM = torch.concat(
                            [torch.zeros(predLSIMStart, device=device), seqLSIM]
                        )
                ...
                if self.p_l.predLSIM > 0 and fadePredWeight > 0:
                    lossPredLSIM = (
                        self.p_l.predLSIM * fadePredWeight * torch.mean(seqLSIM[1:])
                    )
            ...
            if self.p_l.regMeanStd > 0:
                if self.dimension == 2:
                    meanDiff = torch.abs(
                        groundTruth.mean((3, 4)) - prediction.mean((3, 4))
                    )
                    stdDiff = torch.abs(
                        groundTruth.std((3, 4)) - prediction.std((3, 4))
                    )
                elif self.dimension == 3:
                    meanDiff = torch.abs(
                        groundTruth.mean((3, 4, 5)) - prediction.mean((3, 4, 5))
                    )
                    stdDiff = torch.abs(
                        groundTruth.std((3, 4, 5)) - prediction.std((3, 4, 5))
                    )
            ...
            if (
                self.p_l.regVae > 0
                and vaeMeanVar[0] is not None
                and vaeMeanVar[1] is not None
            ):
            ...
            if self.p_l.regLatStep > 0 and latentSpace.shape[1] > 1:
                ...
                lossRegLatStep = self.p_l.regLatStep * torch.mean(
                    torch.abs(latFirst - latSecond)
                )
