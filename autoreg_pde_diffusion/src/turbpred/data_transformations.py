...
class Transforms(object):
    ...
    def __init__(self, p_d: DataParams):
        ...
        assert all(
            aug in ["normalize", "flip", "crop", "resize"] for aug in p_d.augmentations
        ), "Invalid augmentation provided!"
        assert not (
            "crop" in p_d.augmentations and "resize" in p_d.augmentations
        ...
        if self.dim == 2:
            data_noralized = self.p_d.normalizeMode.lower()
            if ("inc" in data_noralized and "mixed" in data_noralized) or (
                "karman" in data_noralized and "mixed" in data_noralized
            ):
                # ORDER (fields): velocity (x,y), --, pressure, ORDER (params): rey, --, --
                self.normMean = np.array(
                    [0.444969, 0.000299, 0, 0.000586, 550.000000, 0, 0],
                    dtype=np.float32,
                )
                self.normStd = np.array(
                    [0.206128, 0.206128, 1, 0.003942, 262.678467, 1, 1],
                    dtype=np.float32,
                )

            if ("tra" in data_noralized and "mixed" in data_noralized) or (
                "mach" in data_noralized and "mixed" in data_noralized
            ):
                ...
                self.normStd = np.array(
                    [0.216987, 0.216987, 0.145391, 0.119944, 1, 0.118322, 1],
                    dtype=np.float32,
                )

            if "iso" in data_noralized and "single" in data_noralized:
                # ORDER (fields): velocity (x,y,z), pressure, ORDER (params): --, --, --
                self.normMean = np.array(
                    [-0.054618, -0.385225, -0.255757, 0.033446, 0, 0, 0],
                    dtype=np.float32,
                )
                self.normStd = np.array(
                    [0.539194, 0.710318, 0.510352, 0.258235, 1, 1, 1], dtype=np.float32
                )
    ...
    def __call__(self, sample: dict):
        ...
        if torch.utils.data.get_worker_info():
            self.randGen = np.random.RandomState(
                torch.utils.data.get_worker_info().seed % 4294967295
            )
        ...
        if self.normalize:
            ...
            meanData = (
                self.normMean[filterArr].reshape((1, -1, 1, 1))
                if self.dim == 2
                else self.normMean[filterArr].reshape((1, -1, 1, 1, 1))
            )
            stdData = (
                self.normStd[filterArr].reshape((1, -1, 1, 1))
                if self.dim == 2
                else self.normStd[filterArr].reshape((1, -1, 1, 1, 1))
            )
            if self.dim == 2:
        ...
        if self.flip:
            if self.dim == 2:
                ...
                if flipped:
                    data = (
                        data.copy()
                    )  # prevent negative strides that has issues with torch tensor creation
            if self.dim == 3:
        ...
        if self.resize:
            result = F.interpolate(
                result, self.outputSize, mode="bilinear", align_corners=True
            )
            if obsMask is not None:
                obsMask = F.interpolate(
                    obsMask, self.outputSize, mode="nearest", align_corners=True
                )
