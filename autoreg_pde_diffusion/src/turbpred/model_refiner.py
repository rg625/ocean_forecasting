...
class PDERefiner(nn.Module):
    def __init__(self, p_d: DataParams, p_md: ModelParamsDecoder, condChannels: int):
        ...
        self.unet = Unet(
            ...
            + (self.p_d.dimension + len(self.p_d.simFields) + len(self.p_d.simParams)),
            out_dim=self.p_d.dimension
    ...
    def forward(self, conditioning: torch.Tensor, data: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            k = torch.randint(
                0, self.numSteps, (d.shape[0], 1, 1, 1), device=device
            ).long()
            ...
            dataNoisyMasked = torch.where(
                k == 0, torch.zeros_like(dataNoisy), dataNoisy
            )
            ...
            pred = torch.reshape(
                pred, (-1, seqLen, data.shape[2], data.shape[3], data.shape[4])
            )
            target = torch.reshape(
                target, (-1, seqLen, data.shape[2], data.shape[3], data.shape[4])
            )
        ...
        # INFERENCE
            ...
            for i in range(1, self.numSteps + 1):
                ...
                predictedNoise = self.unet(
                    torch.concat((cond, predNoisy), dim=1), kSqueeze
                )
