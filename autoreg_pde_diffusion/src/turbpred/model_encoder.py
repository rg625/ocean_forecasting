...
def reparameterizeAndSampleLatentSpaceVAE(
    latentSpace: torch.Tensor, simParam: torch.Tensor
):
    ...
    epsilon = torch.randn(
        (std.shape[0], 1, std.shape[2]), device="cuda" if std.is_cuda else "cpu"
    ).expand(-1, std.shape[1], -1)
    result = vaeMean + (epsilon * std)
    if simParam is not None:
        result[:, :, 0 : simParam.shape[2]] = (
            simParam  # overwrite sampling for simulation parameters
        )
...
class EncoderModelSkip(nn.Module):
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        if self.dimension == 2:
            ...
            d = torch.reshape(
                data, (-1, data.shape[2], data.shape[3], data.shape[4], data.shape[5])
            )
            # print(d.shape)
...
class DecoderModelSkip(nn.Module):
    ...
    def __init__(
        ...
        if self.dimension == 2:
            ...
            self.decConv5 = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Upsample(
                    size=(max(int(sizeX / 2), 64), max(int(sizeY / 2), 32)),
                    mode="nearest",
                ),
            )
            ...
            self.decConv6 = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Upsample(
                    size=(sizeX, sizeY, sizeZ), mode="trilinear", align_corners=True
                ),
            )
            self.decConv7 = nn.Sequential(
    ...
    def forward(self, data: torch.Tensor, simParam: torch.Tensor) -> torch.Tensor:
        ...
        if self.dimension == 2:
            ...
            lat = torch.unsqueeze(
                torch.unsqueeze(torch.unsqueeze(d, dim=2), dim=3), dim=4
            )
            # print(lat.shape)
            ...
            d = torch.reshape(
                d, (-1, seqLen, d.shape[1], d.shape[2], d.shape[3], d.shape[4])
            )
            # print(d.shape)
