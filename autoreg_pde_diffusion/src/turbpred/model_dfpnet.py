...
def blockUNet(
    ...
    if not transposed:
        block.add_module(
            "%s_conv" % name,
            nn.Conv2d(
                in_c, out_c, kernel_size=size, stride=stride, padding=pad, bias=True
            ),
        )
    else:
        block.add_module(
            "%s_upsam" % name, nn.Upsample(scale_factor=stride, mode="bilinear")
...
class DfpNet(nn.Module):
    def __init__(self, inChannels=3, outChannels=3, blockChannels=6, dropout=0.0):
        super(DfpNet, self).__init__()
        c = blockChannels

        self.layer1 = nn.Sequential()
        self.layer1.add_module(
            "layer1_conv", nn.Conv2d(inChannels, c, 4, 2, 1, bias=True)
        )
        ...
        self.dlayer1.add_module(
            "dlayer1_tconv", nn.ConvTranspose2d(c * 2, outChannels, 4, 2, 1, bias=True)
        )
...
class DfpNetTimeEmbedding(nn.Module):
    def __init__(self, inChannels=3, outChannels=3, blockChannels=6, dropout=0.0):
        ...
        self.dlayer2b = BlockUNetTimeEmb(
            c * 4,
            c * 2,
            "dlayer2b",
            transposed=True,
