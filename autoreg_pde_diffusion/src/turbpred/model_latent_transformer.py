...
class PositionalEncoding(nn.Module):
    def __init__(self, latSize: int, dropout: float = 0.0, maxLen: int = 1000):
        ...
        div = torch.exp(
            torch.arange(0, latSize, 2) * (-math.log(10000.0) / latSize)
        ).unsqueeze(0)
...
class LatentModelTransformerEnc(nn.Module):
    ...
    def __init__(
        ...
        encoderLayer = nn.TransformerEncoderLayer(
            lat, heads, width, dropout, batch_first=True
        )
    ...
    def forward(self, data: torch.Tensor, simParam: torch.Tensor) -> torch.Tensor:
        ...
        if self.p_d.simParams:
            ...
            ]  # remove simulation parameter prediction
        else:
            return d
...
class LatentModelTransformerDec(nn.Module):
    ...
    def __init__(
        self, p_d: DataParams, p_me: ModelParamsEncoder, p_ml: ModelParamsLatent
    ):
    ...
    def forward(
        ...
        data: torch.Tensor,
        target: torch.Tensor,
        simParamData: torch.Tensor,
        simParamTarget: torch.Tensor,
    ) -> torch.Tensor:
        # print()
        # print(data.shape)
        # print(target.shape)
        ...
        d = (
            torch.concat([data, simParamData], dim=2)
            if simParamData is not None
            else data
        )
        t = (
            torch.concat([target, simParamTarget], dim=2)
            if simParamTarget is not None
            else target
        )

        # prepare masks
        maskData = torch.triu(
            torch.ones(seqLenTarget, seqLenData) * float("-inf"), diagonal=1
        )  # note: correct mask shape!
        maskData = maskData.to("cuda" if data.is_cuda else "cpu")
        maskTarget = torch.triu(
            torch.ones(seqLenTarget, seqLenTarget) * float("-inf"), diagonal=1
        )
        ...
        # print(d.shape)
        d = self.transformerDec(
            tgt=t, memory=d, tgt_mask=maskTarget, memory_mask=maskData
        )
        # print(d.shape)
        if self.p_d.simParams:
            return d[
...
class LatentModelTransformer(nn.Module):
    ...
    def forward(
        ...
        # prepare masks
        maskData = torch.triu(
            torch.ones(seqLenData, seqLenData) * float("-inf"), diagonal=1
        )  # note: different mask shape!
        maskData = maskData.to("cuda" if data.is_cuda else "cpu")
        maskTarget = torch.triu(
            torch.ones(seqLenTarget, seqLenTarget) * float("-inf"), diagonal=1
        )
