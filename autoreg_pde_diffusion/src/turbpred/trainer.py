...
class Trainer(object):
    ...
    def __init__(
        ...
        if self.p_t.fadeInSeqLen[0] > 0 and not self.p_t.fadeInSeqLenLin:
            ...
            seqIncStep = math.floor(
                (self.p_t.fadeInSeqLen[1] - self.p_t.fadeInSeqLen[0]) / (numSeqInc - 1)
            )
            self.seqIncreaseSteps = list(
                range(
                    self.p_t.fadeInSeqLen[0], self.p_t.fadeInSeqLen[1] + 1, seqIncStep
                )
            )

            assert numSeqInc == len(
                self.seqIncreaseSteps
            ), "Sequence length computation problem"
            print(
                "Sequence length schedule: %d increases by factor 2 at epochs %s"
                % (numSeqInc, str(self.seqIncreaseSteps))
            )
            print("")
            logging.info(
                "Sequence length schedule: %d increases by factor 2 at epochs %s"
            ...
            seqIncStr = (
                str(self.seqIncreaseSteps)
                if len(self.seqIncreaseSteps) < 10
                else str(self.seqIncreaseSteps[:4])
                + " ... "
                + str(self.seqIncreaseSteps[-4:])
            )
            print(
                "Sequence length schedule: %d increases by value 1 at epochs %s"
                % (numSeqInc, seqIncStr)
            )
            print("")
            logging.info(
    ...
    def trainingStep(self, epoch: int):
        ...
        for s, sample in enumerate(self.trainLoader, 0):
            ...
            prediction, latentSpace, vaeMeanVar = self.model(
                data, simParameters, useLatent=useLatent
            )

            if self.currentSeqLen < self.seqenceLength:
                if vaeMeanVar[0] is not None and vaeMeanVar[1] is not None:
                    vaeMeanVar = (
                        vaeMeanVar[0][:, 0 : self.currentSeqLen],
                        vaeMeanVar[1][:, 0 : self.currentSeqLen],
                    )

                p = prediction[:, 0 : self.currentSeqLen]
                d = data[:, 0 : self.currentSeqLen]
                latents = latentSpace[:, 0 : self.currentSeqLen]
            else:
                p = prediction
                d = data
                latents = latentSpace
            ...
            loss, lossParts, lossSeq = self.criterion(
                p,
                d,
                latents,
                vaeMeanVar,
                fadePredWeight=fadeWeight,
                ignorePredLSIMSteps=ignorePredLSIMSteps,
            )
            ...
            self.trainHistory.updateBatch(
                lossParts, lossSeq, s, (timerEnd - timerStart) / 60.0
            )
    ...
    def testStep(self, epoch: int):
        ...
        with torch.no_grad():
            for s, sample in enumerate(self.testLoader, 0):
                ...
                prediction, latentSpace, vaeMeanVar = self.model(
                    data, simParameters, useLatent=True
                )
                ...
                _, lossParts, lossSeq = self.criterion(
                    p, d, latents, vaeMeanVar, weighted=False
                )

                timerEnd = time.perf_counter()
                self.testHistory.updateBatch(
                    lossParts, lossSeq, s, (timerEnd - timerStart) / 60.0
                )
