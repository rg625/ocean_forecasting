...
class DiffusionModel(nn.Module):
    ...
    def __init__(
        ...
        self.inferenceConditioningIntegration = (
            self.p_md.diffCondIntegration
        )  # "noisy" or "clean"
        ...
        alphasCumprodPrev = F.pad(
            alphasCumprod[:-1], (0, 0, 0, 0, 0, 0, 1, 0), value=1.0
        )
        ...
        if "dfp" in self.p_md.arch:
            self.unet = DfpNetTimeEmbedding(
                inChannels=condChannels
                + (
                    self.p_d.dimension
                    + len(self.p_d.simFields)
                    + len(self.p_d.simParams)
                ),
                outChannels=condChannels
                + (
                    self.p_d.dimension
                    + len(self.p_d.simFields)
                    + len(self.p_d.simParams)
    ...
    def forward(self, conditioning: torch.Tensor, data: torch.Tensor) -> torch.Tensor:
        ...
        cond = torch.reshape(
            conditioning,
            (-1, conditioning.shape[2], conditioning.shape[3], conditioning.shape[4]),
        )
        ...
        if self.training:
            ...
            if self.p_md.diffCondIntegration == "noisy":
                ...
                t = torch.randint(
                    0, self.timesteps, (d.shape[0],), device=device
                ).long()
                dNoisy = (
                    self.sqrtAlphasCumprod[t] * d
                    + self.sqrtOneMinusAlphasCumprod[t] * noise
                )
                ...
                dNoisy = (
                    self.sqrtAlphasCumprod[t] * d
                    + self.sqrtOneMinusAlphasCumprod[t] * dNoise
                )
            ...
            noise = torch.reshape(
                ...
                (
                    -1,
                    seqLen,
                    conditioning.shape[2] + data.shape[2],
                    data.shape[3],
                    data.shape[4],
                ),
            )
        ...
        # INFERENCE
            ...
            if self.inferenceInitialSampling == "random":
                ...
                dNoise = torch.randn(
                    (1, d.shape[1], d.shape[2], d.shape[3]), device=device
                ).expand(d.shape[0], -1, -1, -1)
                cNoise = torch.randn(
                    (1, cond.shape[1], cond.shape[2], cond.shape[3]), device=device
            ...
            for i in reversed(range(0, self.timesteps, sampleStride)):
                ...
                if self.inferenceConditioningIntegration == "noisy":
                    condNoisy = (
                        self.sqrtAlphasCumprod[t] * cond
                        + self.sqrtOneMinusAlphasCumprod[t] * cNoise
                    )
                else:
                    condNoisy = cond
                ...
                modelMean = self.sqrtRecipAlphas[t] * (
                    ...
                    - self.betas[t]
                    * predictedNoiseCond
                    / self.sqrtOneMinusAlphasCumprod[t]
                )

                dNoise = modelMean[
                    :, cond.shape[1] : modelMean.shape[1]
                ...
                if i != 0 and self.inferenceSamplingMode == "ddpm":
                    if self.inferencePosteriorSampling == "random":
                        ...
                        dNoise = dNoise + self.sqrtPosteriorVariance[
                            t
                        ] * torch.randn_like(dNoise)
                    else:
            ...
            # unstack batch and sequence dimension again
            dNoise = torch.reshape(
                dNoise, (-1, seqLen, data.shape[2], data.shape[3], data.shape[4])
            )
