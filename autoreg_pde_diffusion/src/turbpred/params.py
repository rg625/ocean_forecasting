class DataParams(object):
    def __init__(
        ...
        self.sequenceLength = (
            sequenceLength  # number of simulation frames in one sequence
        )
        self.randSeqOffset = randSeqOffset  # randomize sequence starting frame
        self.dataSize = (
            dataSize  # target data size for scale/crop/cropRandom transformation
        )
        ...
        self.simParams = (
            simParams  # which simulation parameters are added from ["rey", "mach"]
        )
...
class TrainingParams(object):
    def __init__(
        ...
        self.fadeInPredLoss = (
            fadeInPredLoss  # start and end epoch of fading in the prediction loss terms
        )
        self.fadeInSeqLen = (
            fadeInSeqLen  # start and end epoch of fading in the sequence length
        )
...
class LossParams(object):
    def __init__(
        ...
        self.regMeanStd = (
            regMeanStd  # mean and standard deviation regularization weight
        )
...
class ModelParamsEncoder(object):
    def __init__(
        self, arch="skip", pretrained=False, frozen=False, encWidth=16, latentSize=16
    ):
