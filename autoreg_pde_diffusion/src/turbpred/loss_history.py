...
class LossHistory(object):
    ...
    def updateBatch(self, lossParts: dict, lossSeq: dict, sample: int, timeMin: float):
        ...
        toPrint = (
            ...
            and sample % self.printInterval == self.printInterval - 1
        )
        toLog = (
            self.logInterval > 0 and sample % self.logInterval == self.logInterval - 1
        )
    ...
    def updateEpoch(self, timeMin: float):
        ...
        print(
            "%s Epoch %d (%2.2f min): %1.4f    %s"
            % (self.modeLong, self.epoch + 1, timeMin, loss, partStr)
        )
        print("")
        logging.info(
            "%s Epoch %d (%2.2f min): %1.4f    %s"
    ...
    def updateAccuracy(
        self, params: List, otherHistories: List["LossHistory"], finalPrint: bool
    ):
        par = {}
        for p in params:
            if p:
    ...
    def writePredictionExample(
        self, prediction: torch.Tensor, groundTruth: torch.Tensor
    ):
        ...
        for i in range(len(data)):
            ...
            self.writer.add_images(
                "%s_PredictionImg/%s" % (self.mode, names[i]), img, self.epoch
            )
            self.writer.add_video(
                "%s_PredictionVid/%s" % (self.mode, names[i]), d, self.epoch, fps=5
            )

    def writeSequenceLoss(self, lossSeq: dict):
        mse = lossSeq["MSE"].cpu().numpy()
        if lossSeq["LSIM"] is not None:
        ...
        self.writer.add_figure(
            "%s_PredictionImg/errorSequence" % (self.mode), fig, self.epoch
        )
