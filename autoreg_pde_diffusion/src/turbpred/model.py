...
from turbpred.model_latent_transformer import (
...
class PredictionModel(nn.Module):
    ...
    def __init__(
        ...
        if (
            (self.p_me and self.p_me.pretrained)
            or (self.p_md and self.p_md.pretrained)
            or (self.p_ml and self.p_ml.pretrained)
        ):
            if pretrainPath:
        ...
        if self.p_me:
            ...
            if pretrainPath and self.p_me.pretrained:
                self.modelEncoder.load_state_dict(
                    loadedPretrainedWeightDict["stateDictEncoder"]
                )
        ...
        if self.p_md:
            if self.p_md.arch == "skip":
                self.modelDecoder = DecoderModelSkip(
                    p_d, p_me, p_md, p_ml, p_d.dimension
                )

            elif self.p_md.arch in [
                "unet",
                "unet+Prev",
                "unet+2Prev",
                "unet+3Prev",
                ...
                inChannels = prevSteps * (
                    self.p_d.dimension
                    + len(self.p_d.simFields)
                    + len(self.p_d.simParams)
                )
                outChannels = (
                    self.p_d.dimension
                    + len(self.p_d.simFields)
                ...
                if "unet" in self.p_md.arch:
                    ...
                    self.modelDecoder = DilatedResNet(
                        inFeatures=inChannels,
                        outFeatures=outChannels,
                        blocks=4,
                        features=self.p_md.decWidth,
                        dilate="dil_" in self.p_md.arch,
                    )

                elif "fno" in self.p_md.arch:
                    self.modelDecoder = FNO(
                        n_modes=(self.p_md.fnoModes[0], self.p_md.fnoModes[1]),
                        hidden_channels=self.p_md.decWidth,
                ...
                if self.p_md.arch in ["decode-ddpm", "decode-ddim"]:
                    ...
                    condChannels = (
                        ...
                        + len(self.p_d.simFields)
                        + len(self.p_d.simParams)
                    ) + (self.p_me.latentSize + len(self.p_d.simParams))
                self.modelDecoder = DiffusionModel(
                    p_d, p_md, p_d.dimension, condChannels=condChannels
                )
                ...
                self.modelDecoder = nn.ModuleList(
                    [
                        ...
                        DiffusionModel(
                            ...
                            p_md,
                            p_d.dimension,
                            condChannels=self.p_d.dimension
                            + len(self.p_d.simFields)
                            + len(self.p_d.simParams),
                        ),
                    ]
            ...
            if pretrainPath and self.p_md.pretrained:
                if self.p_md.arch in [
                    "skip+finetune-ddpm",
                    "skip+finetune-ddim",
                    "skip+hybrid-ddpm",
                    "skip+hybrid-ddim",
                ]:
                    self.modelDecoder[0].load_state_dict(
                        loadedPretrainedWeightDict["stateDictDecoder"]
                    )
                else:
                    self.modelDecoder.load_state_dict(
                        loadedPretrainedWeightDict["stateDictDecoder"]
                    )
        ...
        if self.p_ml:
            ...
            if pretrainPath and self.p_ml.pretrained:
                self.modelLatent.load_state_dict(
                    loadedPretrainedWeightDict["stateDictLatent"]
                )
    ...
    def forward(
        ...
        if self.p_md.arch not in [
            ...
            if not self.modelLatent or not useLatent:
                ...
                if isinstance(self.modelLatent, LatentModelTransformerEnc):
                    ...
                    self.modelLatent, LatentModelTransformerDec
                ) or isinstance(self.modelLatent, LatentModelTransformer):
        ...
        # DIRECT PREDICTION OF NEXT FRAME WITH DIFFERENT ARCHITECTURES
            ...
            if (
                isinstance(self.modelDecoder, Unet)
                or isinstance(self.modelDecoder, DilatedResNet)
                or isinstance(self.modelDecoder, FNO)
            ):
                ...
                if stepsLong > 0 and (not self.training):
                    prediction = self.forwardDiffusionDirectLongGPUEfficient(
                        d, simParam, steps=stepsLong
                    )
                else:
    ...
    def forwardTransEnc(
        self, d: torch.Tensor, latentSpace: torch.Tensor, simParam: torch.Tensor
    ) -> torch.Tensor:
        sizeSeq = d.shape[1]
        ...
        if self.training and not self.p_ml.transTrainUnroll:
            ...
            latentSpace = torch.concat(
                [encLatentSpace[:, :1], transLatentSpace[:, :-1]], dim=1
            )
        ...
        # transformer encoder latent model to predict all steps from first one
            ...
            for i in range(1, sizeSeq):
                start = (
                    max(0, i - self.p_ml.maxInputLen)
                    if self.p_ml.maxInputLen > 0
                    else 0
                )
                transLatentSpace = self.modelLatent(
                    latentSpace[:, start:i],
                    simParam[:, start:i] if simParam is not None else None,
                )
    ...
    def forwardTransDec(
        ...
        if self.training and not self.p_ml.transTrainUnroll:
            ...
            transLatentSpace = self.modelLatent(
                encLatentSpace[:, :-1],
                encLatentSpace[:, 1:],
                simParam[:, :-1] if simParam is not None else None,
                simParam[:, 1:] if simParam is not None else None,
            )
            ...
            for i in range(1, sizeSeq):
                if self.p_ml.transTargetFull:
                    ...
                    transLatentSpace = self.modelLatent(
                        latentSpace[:, :i],
                        latentSpace[:, i - 1 : i],
                        simParam[:, :i] if simParam is not None else None,
                        simParam[:, i - 1 : i] if simParam is not None else None,
                    )
    ...
    def forwardTransMGN(
        ...
        if simParam is not None:
            latentSpace[:, 0] = self.modelLatentParamEmb(
                simParam[:, 0]
            )  # only use scalar simParam input
        latentSpace[:, 1:2] = self.modelEncoder(d[:, 0:1])
        for i in range(2, sizeSeq + 1):
            start = (
                max(1, i - self.p_ml.maxInputLen) if self.p_ml.maxInputLen > 0 else 1
            )
            transInput = torch.concat(
                [latentSpace[:, 0:1], latentSpace[:, start:i]], dim=1
            )
    ...
    def forwardDirect(self, d: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(prevSteps):  # no prediction of first steps
            if self.training:
                trainNoise = self.p_md.trainingNoise * torch.normal(
                    torch.zeros_like(d[:, i]), torch.ones_like(d[:, i])
                )
        ...
        for i in range(prevSteps, sizeSeq):
            ...
            if self.p_d.simParams:
                result[:, -len(self.p_d.simParams) :] = d[
                    :, i, -len(self.p_d.simParams) :
    ...
    def forwardDirectLongGPUEfficient(
        self, d: torch.Tensor, steps: int
    ) -> torch.Tensor:
        _, sizeSeq = d.shape[0], steps

        if (
            "+Prev" in self.p_md.arch
            or "+2Prev" in self.p_md.arch
            or "+3Prev" in self.p_md.arch
        ):
    ...
    def forwardDiffusionDirect(
        self, d: torch.Tensor, simParams: torch.Tensor
    ) -> torch.Tensor:
        _, sizeSeq = d.shape[0], d.shape[1]
        ...
        if self.training:
            if "+Enc" in self.p_md.arch:
                ...
                latents = (
                    torch.concat((latentSpace, simParams[:, 0:1]), dim=2)
                ...
                conditioning = (
                    latents.unsqueeze(3)
                    .unsqueeze(4)
                    .expand(-1, -1, -1, d.shape[3], d.shape[4])
                )

                randIndex = torch.randint(1, sizeSeq, (1,), device=d.device)
                conditioning = torch.concat(
                    (conditioning, d[:, randIndex - 1 : randIndex]), dim=2
                )
                data = d[:, randIndex]
                ...
                randIndex = torch.randint(1, sizeSeq, (1,), device=d.device)
                conditioning = torch.concat(
                    (d[:, 0:1], d[:, randIndex - 1 : randIndex]), dim=2
                )
            ...
            noise, predictedNoise = self.modelDecoder(
                conditioning=conditioning, data=data
            )
    ...
    def forwardDiffusionDirectLongGPUEfficient(
        self, d: torch.Tensor, simParams: torch.Tensor, steps: int
    ) -> torch.Tensor:
        _, sizeSeq = d.shape[0], steps
        ...
        # INFERENCE
            ...
            for i in range(2, sizeSeq):
                ...
                result = self.modelDecoder(
                    conditioning=cond, data=torch.zeros_like(uIn, device="cuda")
                )  # auto-regressive inference

                if self.p_d.simParams:
                    result[:, :, -len(self.p_d.simParams) :] = d[
                        :, 0:1, -len(self.p_d.simParams) :
                    ].to(
                        "cuda"
                    )  # replace simparam prediction with true values
    ...
    def forwardDiffusionDecode(
        self, d: torch.Tensor, latentSpace: torch.Tensor, simParams: torch.Tensor
    ) -> torch.Tensor:
        # add simulation parameters to latent space
        latents = (
            torch.concat((latentSpace, simParams), dim=2)
            if simParams is not None
            else latentSpace
        )
        ...
        cond = (
            latents.unsqueeze(3).unsqueeze(4).expand(-1, -1, -1, d.shape[3], d.shape[4])
        )

        if self.training:
            noise, predictedNoise = self.modelDecoder(
                conditioning=cond, data=d
            )  # prediction conditioned on latent space
            return noise, predictedNoise
        else:
    ...
    def forwardDiffusionHybrid(
        ...
        if self.training:
            if "+Lat" in self.p_md.arch:
                ...
                conditioning = torch.concat(
                    (
                        conditioning[:, randIndex : randIndex + 1],
                        d[:, randIndex - 1 : randIndex],
                    ),
                    dim=2,
                )
                data = d[:, randIndex : randIndex + 1]
                ...
                conditioning = torch.concat(
                    (
                        predictionAeDec[:, randIndex : randIndex + 1],
                        d[:, randIndex - 1 : randIndex],
                    ),
                    dim=2,
                )
                ...
                noise, predictedNoise = self.modelDecoder[1](
                    conditioning=conditioning, data=data
                )
            ...
            if "+Lat" in self.p_md.arch:
                ...
                for i in range(1, sizeSeq - 1):
                    cond = torch.concat(
                        (conditioning[:, i : i + 1], prediction[:, i - 1 : i]), dim=2
                    )
                    result = self.modelDecoder(conditioning=cond, data=d[:, i - 1 : i])
                    if self.p_d.simParams:
                        result[:, :, -len(self.p_d.simParams) :] = d[
                ...
                for i in range(1, sizeSeq - 1):
                    cond = torch.concat(
                        (predictionAeDec[:, i : i + 1], prediction[:, i - 1 : i]), dim=2
                    )
                    result = self.modelDecoder[1](
                        conditioning=cond, data=d[:, i - 1 : i]
                    )
                    if self.p_d.simParams:
                        result[:, :, -len(self.p_d.simParams) :] = d[
    ...
    def printModelInfo(self):
        ...
        if self.modelEncoder:
            pTrainEnc = filter(
                lambda p: p.requires_grad, self.modelEncoder.parameters()
            )
        ...
        if self.modelDecoder:
            pTrainDec = filter(
                lambda p: p.requires_grad, self.modelDecoder.parameters()
            )
        ...
        print(
            "Weights Trainable (All): %d (%d)   %s   %s   %s"
            % (
                paramsTrain,
                params,
                (
                    ("Enc: %d (%d)" % (paramsTrainEnc, paramsEnc))
                    if self.modelEncoder
                    else ""
                ),
                (
                    ("Dec: %d (%d)" % (paramsTrainDec, paramsDec))
                    if self.modelDecoder
                    else ""
                ),
                (
                    ("Lat: %d (%d)" % (paramsTrainLat, paramsLat))
                    if self.modelLatent
                    else ""
                ),
            )
    ...
    def load(cls, path: str, useGPU: bool = True):
        ...
        p_me = (
            ModelParamsEncoder().fromDict(loaded["modelParamsEncoder"])
            if loaded["modelParamsEncoder"]
            else None
        )
        p_md = (
            ModelParamsDecoder().fromDict(loaded["modelParamsDecoder"])
            if loaded["modelParamsDecoder"]
            else None
        )
        p_ml = (
            ModelParamsLatent().fromDict(loaded["modelParamsLatent"])
            if loaded["modelParamsLatent"]
            else None
        )
        p_d = (
            DataParams().fromDict(loaded["dataParams"])
            if loaded["dataParams"]
            else None
        )
        p_t = (
            TrainingParams().fromDict(loaded["trainingParams"])
            if loaded["trainingParams"]
            else None
        )
        p_l = (
            LossParams().fromDict(loaded["lossParams"])
            if loaded["lossParams"]
            else None
        )
    ...
    def save(self, basePath: str, epoch: int = -1, noPrint: bool = False):
        ...
        saveDict = {
            "stateDictEncoder": (
                self.modelEncoder.state_dict() if self.modelEncoder else None
            ),
            "stateDictDecoder": (
                self.modelDecoder.state_dict() if self.modelDecoder else None
            ),
            "stateDictLatent": (
                self.modelLatent.state_dict() if self.modelLatent else None
            ),
