...
def getColor(id: str):
    if id not in colorRemap:
        ...
        print(
            "WARNING: no plotting color found for model with ID: %s, using color %s instead"
            % (id, col)
        )
        return col
    else:
...
def getColormapAndNorm(dataset: str, field: str):
    ...
    if field == "vort":
        if dataset in ["lowRey", "highRey", "varReyIn", "stable"]:
            ...
            newcolors = np.vstack(
                (top(np.linspace(0.1, 1, 128)), bottom(np.linspace(0, 0.9, 128)))
            )
