...
for modelName, modelPath in models.items():
    ...
    if datasetName in ["extrap", "interp", "longer"]:
        ...
        yPos = int(
            (cylsY / 3.0) * 0.5 * prediction.shape[6] + 0.5 * prediction.shape[6]
        )
    elif datasetName in ["highRey", "lowRey", "varReyIn"]:
        xPos = int(((0.6 * cylsDownstream + 1.3) / 4.0) * prediction.shape[5])
        yPos = int(
            (0.6 * cylsY) * 0.5 * prediction.shape[6] + 0.5 * prediction.shape[6]
        )
    ...
    n = fft.shape[3]
    gridSpacing = (
        0.002 if datasetName in ["zInterp"] else 1
    )  # delta t between frames as 1 here
...
if datasetName in ["extrap", "interp", "longer", "highRey", "lowRey", "varReyIn"]:
    ...
    ax.set_xlabel("Temporal %s Frequency" % (field))
# ax.set_ylabel("Amplitude * Frequency")
ax.set_ylabel("$v_y$ Amplitude $*f^2$")
...
for i in range(len(modelNames)):
    ...
    if modelNames[i] == "Simulation":
        ms = np.logspace(4, 0.1, freq.shape[0], base=2)
