...
fig, axs = plt.subplots(
    nrows=len(timeSteps), ncols=5, squeeze=0, figsize=(5.8, 1.4), dpi=250
)
...
axs[len(timeSteps) - 1, 0].set_xlabel(
    r"Sim. ($%d\times%d$)"
    % (spatialZoom[0][1] - spatialZoom[0][0], spatialZoom[1][1] - spatialZoom[1][0])
)
for i in range(1, 4):
...
fig.savefig("%s/data_posterior_%s_%s_2.pdf" % (outputFolder, datasetName, field))
