...
from models.azencot_loss import AzencotLoss, AzencotDiscreteLoss
...
def main(cfg: DictConfig):
    ...
    try:
        ...
        if is_ddp:
            # The Azencot losses consume neither `reynolds` nor the disturbed latents, so
            # the auxiliary Re head receives no gradient and default DDP would abort.
    ...
    # Detect loss type: discrete has 'lamb', continuous has 'beta'
    is_discrete_loss = "lamb" in cfg.loss and cfg.loss.get("lamb") is not None

    if is_discrete_loss:
        # Use discrete loss for azencot/discrete operators
        criterion = AzencotDiscreteLoss(
            to_unit_range=(
                train_dataset.to_unit_range if cfg.loss.get("ssim_weight") else None
            ),
            **cfg.loss,
        )
    else:
        # Use continuous loss for continuous operator
        if cfg.loss.get("ssim_weight", None) is not None:
            criterion = AzencotLoss(
                to_unit_range=train_dataset.to_unit_range, **cfg.loss
            )
        else:
            criterion = AzencotLoss(**cfg.loss)
