...
class EvalConfig:
    ...
    model_type: str = "mlp"
    dimension: int = 128
    regime: str = "tra"  # Used for base directory grouping (e.g. ks, tra)
    ckpt_index: int = 199

    train_rollout: int = 1  # The rollout horizon this model was trained on
    eval_rollout_steps: int = 100  # How many steps to unroll during this evaluation
    ...
    def checkpoint_path(self) -> Path:
        # Dynamically find the latest run directory inside the rollout folder
        print(f"base_output_dir: {self.base_output_dir}")
        exp_dir = (
            self.base_output_dir
            / self.experiment_name
            / f"rollout_{self.train_rollout}"
        )
...
class KoopmanEvaluator:
    ...
    def _initialize(self):
        logger.info(
            f"Initializing Evaluator for train rollout {self.cfg.train_rollout}: {self.cfg.experiment_name}"
        )
    ...
    def run_validation_rollouts(self) -> Tuple[np.ndarray, List[float]]:
        """Runs the rollouts and returns (concatenated_predictions, list_of_individual_mses)"""
        max_samples = min(500, len(self.val_dataset))
        logger.info(f"Starting Rollouts for {max_samples} samples...")

        eval_arrays = []
        sample_mses = []  # <--- Track individual sample MSEs for statistical bounds
        ...
        with torch.inference_mode():
            for idx in tqdm(range(max_samples), desc="Processing Val Set"):
                ...
                # Calculate MSE for this specific sample
                sample_mse_avg = 0.0
    ...
    def save_results(self, data: np.ndarray):
        ...
        filename = f"rollout_{self.cfg.train_rollout}_{self.cfg.model_type}_{self.cfg.model_arch}_{self.cfg.dimension}_{self.cfg.regime}.npz"
...
def main():
    parser = argparse.ArgumentParser(
        description="Koopman Autoencoder Evaluation over Train Rollouts"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="List of rollout horizons models were trained on (e.g. 1 2 3)",
    )
    parser.add_argument(
        "--eval_steps",
        type=int,
    ...
    for rollout_val in args.train_rollouts:
        logger.info("=" * 60)
        logger.info(
            f"Evaluating Model Trained on Rollout: {rollout_val} (Eval steps: {args.eval_steps})"
        )
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.info(
                f"Rollout {rollout_val} | Mean MSE: {mean_mse:.6f} | 95% CI: +/- {margin_of_error:.6f}"
            )

            results_report[rollout_val] = {
                "mean": mean_mse,
                "std": std_mse,
            ...
            logger.exception(f"Evaluation failed for rollout {rollout_val}")
            continue
    ...
    if results_report:
        ...
        sorted_rollouts = sorted(results_report.keys())
        means = [results_report[r]["mean"] for r in sorted_rollouts]
        ci_lower = [results_report[r]["ci_lower"] for r in sorted_rollouts]
        ci_upper = [results_report[r]["ci_upper"] for r in sorted_rollouts]
        stds = [results_report[r]["std"] for r in sorted_rollouts]

        # 1. Save Highly Detailed CSV Report
        csv_path = out_dir / f"mse_report_{args.regime}.csv"
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["Train Rollout", "Mean MSE", "Std Dev", "95% CI Lower", "95% CI Upper"]
            )
            for r in sorted_rollouts:
                writer.writerow(
                    [
                        r,
                        results_report[r]["mean"],
                        results_report[r]["std"],
                        results_report[r]["ci_lower"],
                        results_report[r]["ci_upper"],
                    ]
                )
        logger.info(f"Saved CSV Report to: {csv_path}")

        # 2. Generate Shaded Area Plot
        plt.style.use("seaborn-v0_8-whitegrid")

        fig, ax = plt.subplots(figsize=(8, 5))

        ax.plot(sorted_rollouts, means, marker="o", linewidth=2, label="Mean MSE")

        ax.fill_between(sorted_rollouts, ci_lower, ci_upper, alpha=0.2)

        ax.set_yscale("log")

        ax.set_xlabel("Training Rollout Length", fontsize=12, fontweight="bold")
        ax.set_ylabel("Denormalized MSE", fontsize=12, fontweight="bold")
        ax.set_title(
            f"Evaluation over {args.eval_steps} steps", fontsize=14, fontweight="bold"
        )
        ...
        plt.savefig(
            out_dir / f"mse_plot_{args.regime}.pdf", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            out_dir / f"mse_plot_{args.regime}.png", dpi=300, bbox_inches="tight"
        )
        # --- Save ---
        # plot_path_pdf = out_dir / f"latent_mse_plot_{args.regime}.pdf"
        # plot_path_png = out_dir / f"latent_mse_plot_{args.regime}.png"
