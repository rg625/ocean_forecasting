...
class Validator(object):
    def __init__(self, model, valLoader, criterion):
        ...
        self.histDistMean: List[float] = []
        self.histDistStd: List[float] = []
        self.histCorMean: List[float] = []
        self.histCorStd: List[float] = []
        self.histCorFull: List[float] = []
    ...
    def validationStep(self):
        ...
        with torch.no_grad():
            ...
            distance_arr = np.vstack(distance)
            distMean = float(distance_arr.mean())
            distStd = float(distance_arr.std())
            self.histDistMean.append(distMean)
            self.histDistStd.append(distStd)

            correlation_arr = np.vstack(correlation)
            corMean = float(correlation_arr.mean())
            corStd = float(correlation_arr.std())
