...
class Trainer:
    ...
    def __init__(
        self,
        model: Union[
            KoopmanAutoencoder, nn.Module
        ],  # Broadened type hint to accept ViT
        ...
        # --- Curriculum Config ---
        warmup_epochs: int = 20,  # Phase 1: AE Only
        operator_epochs: int = 20,  # Phase 2: Operator Only
    ):
        self.model = model
        ...
        self.operator_epochs = operator_epochs
        self.phase = "init"  # init, manifold, operator, joint
        ...
        self.cfg_loss = criterion.cfg
        self.original_alpha = criterion.cfg.alpha  # Prediction is now the only goal
        self.original_beta = (
            criterion.cfg.beta
        )  # Stronger latent structure (Linearity check)
        self.original_physics_weight = criterion.cfg.physics_weight

        # 2. Physics Sub-Weights
        self.original_gamma_space = (
            criterion.cfg.gamma_space
        )  # Decoder is frozen, spatial gradients are fixed.
        self.original_gamma_spectral = (
            criterion.cfg.gamma_spectral
        )  # Now penalizes Phase Drift in latent space
        self.original_gamma_time = (
            criterion.cfg.gamma_time
        )  # CRITICAL: Match the velocity (d/dt) exactly.
    ...
    def get_model_module(self):
        """Safely extract the underlying model, handling DDP wrapper."""
        return (
            self.model.module
    ...
    def true_latent_encoding(self, target_td: TensorDict, model_module):
        # Safe-guard for ViT which lacks an explicit encoder
        if not hasattr(model_module, "present_encoding"):
            return None
    ...
    def _run_one_epoch(self) -> Dict[str, float]:
        ...
        for input_td, target_td in self.train_loader:
            ...
            with autocast(
                ...
                # Safely get koopman_operator (will be None for ViT)
                operator = getattr(model_module, "koopman_operator", None)
                true_lats = self.true_latent_encoding(
                    target_td=target_td, model_module=model_module
                )

                loss = self.criterion(
                    koopman_operator=operator,
                    x_recon=out.x_recon,
                    x_preds=out.x_preds,
                    latent_pred=out.z_preds,
                    x_true=x_true_recon,  # Compare against CLEAN input
                    x_future=target_td,  # Compare against CLEAN target
                    true_latents=true_lats,
            ...
            # Normal loss is ~0.02. A spike to 100.0+ is a collapse risk.
            ...
            for i, (input_td, target_td) in enumerate(dataloader):
                ...
                if self.is_main_process() and i < self.num_visual_batches:
                    denormalize_and_visualize(
                        input=dataloader.denormalize(input_td.squeeze()),
                        target=dataloader.denormalize(target_td.squeeze()),
                        x_recon=dataloader.denormalize(out.x_recon.squeeze()),
                        x_preds=dataloader.denormalize(out.x_preds.squeeze()),
    ...
    def _log_metrics(self, metrics: Dict, epoch: int, mode: str):
        ...
        # Safely log timings if the model provides them
        if hasattr(self.model, "timings"):
    ...
    def save_checkpoint(self, epoch: int, val_loss: float, is_best: bool):
        ...
        model_to_save = self.get_model_module()
        state = {
            "epoch": epoch,
