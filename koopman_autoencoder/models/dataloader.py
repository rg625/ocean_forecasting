...
class QGDataset(Dataset):
    def __init__(self, data_path, max_sequence_length=2, variables=None):
        ...
        self.stacked_data = torch.stack(
            [torch.FloatTensor(self.data[var].values) for var in variables], dim=1
        )
        ...
        self.stacked_data = (
            2
            * (self.stacked_data - self.mins[None, :, None, None])
            / (self.maxs[None, :, None, None] - self.mins[None, :, None, None])
            - 1
        )
    ...
    def denormalize(self, x):
        ...
        return (
            0.5 * (x + 1) * (maxs[None, :, None, None] - mins[None, :, None, None])
            + mins[None, :, None, None]
        )
